#include "RiceStyle.h"
using namespace std;
void plot_diffractive_e_resolution(TString filename="output/eicrecon-sartre_coherent_phi_output.root", int num=0){

	TFile* file = new TFile(filename);	TString vm_label="#phi #rightarrow K^{+}K^{-}";
	...
	double xmax=18.;
	if(num==1) xmax=2.0;
	TString xaxis_label="E (GeV)";
	if(num==1) xaxis_label="p_{T} (GeV/c)";
	TString yaxis_label="RMS of #delta E/E (resolution)";
	if(num==1) yaxis_label="RMS of #delta p_{T}/p_{T} (resolution)";

	if(num==0) vm_label="reco'd e'";
	...
	TH1D* base1 = makeHist("base1", "", xaxis_label, yaxis_label, 100,xmin,xmax,kBlack);
	base1->GetYaxis()->SetRangeUser(1e-2, 2);
	...
	base1->Draw();
	TH2D* h_res;
	if(num==0) h_res=(TH2D*) h_energy_res;
	if(num==1) h_res=(TH2D*) h_VM_res;


	TH1D* h_res_1D = new TH1D("h_res_1D","",100,xmin,xmax);
	...
	TLatex* r44_2 = new TLatex(0.15, 0.18, vm_label);
	r44_2->SetNDC();
	...
	drawPad->SetLogz(1);
	drawPad->cd();
	TH1D* base2 = makeHist("base2", "", "MC", " resolution ", 100,xmin,xmax,kBlack);
	base2->GetYaxis()->SetRangeUser(-1.5, 1.5);
	...
	if(num==0) c2->Print("./figures/benchmark-e'-E-resolution-from-"+outputfilename+".pdf");
	if(num==1) c2->Print("./figures/benchmark-VM-pT-resolution-from-"+outputfilename+".pdf");
